...
namespace Kunukn.XmlVisual.Core.SvgBuilder.Factory
{
    ...
    public class SvgChineseBoxBuilder : SvgBuilder
    {
        // singleton
        private static SvgBuilder _obj;
        private static readonly object Padlock = new object();
        private SvgChineseBoxBuilder() { }
        public static SvgBuilder Instance //thread safe
        {
            get
            {
                lock (Padlock)
                {
                    return _obj ?? (_obj = new SvgChineseBoxBuilder());
                }
            }
        }
        // end singleton        
        ...
        public override SvgFile BuildSvg(XmlMetadata data)
        {
            ...
            A = 20;
            //B = UConfig.IncludeBigraph ? 5 * SvgTool.ElementHeight : 3 * SvgTool.ElementHeight;
            ...
            for (int i = 1; i <= data.Height; i++)
            {
                ...
                foreach (var e in level)
                {
                    ...
                    j++;
                    if (parent != e.Parent) //new parent, change offset
                    {
                        offset = 0;
                        parent = e.Parent;
                    }
                    e.Svg.X = e.Parent.Svg.X + offset + Padding / 2;
                    e.Svg.Y = i * SvgTool.ElementHeight;
                    boxes.Append(CreateElementGroup(e, A, B));
                    offset += e.Svg.Width + Padding;
                }
            }

            //             adjust, bigraph use real pos of element x,y            
            foreach (var e in data.AllElements)
            {
                e.Svg.X += A;
                e.Svg.Y += B;
            }
        ...
        void DoCalculation(Element startRoot, XmlMetadata data)
        {
...
//            RecursiveFindMaxWidth(startRoot, 1, max);
          ...
          //  RecursiveSetSize(startRoot, 1, totalWidth);
            ...
            foreach (var e in all)
            {
                int steps = 1 + data.Height - e.Level;
                //                if (!e.IsLeaf())
                e.Svg.Height += steps * (Padding + e.Svg.Height);
            }



            if (startRoot != null) // update            
